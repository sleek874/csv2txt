...
- 新增、取消、移除、復原與清空各自具有明確且互斥的操作狀態。移除與復原必須先在 worker 完成，再公開對應的清單與自動預覽；清空可中止任何主要工作區操作，舊操作結果不得回寫。
...
- 「全部標示為已查看」、處理取消與移除復原都是固定資訊區的情境操作。資訊區右側保留兩個固定操作欄位：全部已查看固定在左欄，只要仍有未查看檔案便持續顯示；右欄只顯示當下操作，包括未加入明細、取消或復原。檔名、數量、按鈕顯示與隱藏都不得改變欄位位置。處理與取消中只顯示取消，不移動一般完成焦點，取消完成後焦點回到「新增檔案」。
...
- 下載卡固定依序顯示狀態、檔案／已勾選列摘要及一句下一步；問題或提醒預設收合，由與預覽相同的 `+／−` disclosure 展開完整清單。
...
- 進階下載卡與 Section 2 使用相同的狀態、檔案／列摘要、簡短下一步與預設收合提醒清單。Reference 的空白標題、重複標題與未儲存公式結果依類型合併為有界的白話摘要；讀取提醒、重複與未找到參照資料都不阻止進階下載。只有來源仍在解析、沒有勾選列或 reference workbook 無法解析時不能建立結果。
