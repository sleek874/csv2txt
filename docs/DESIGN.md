...
- 檔案選擇區只保留固定說明與「新增檔案」、「清空清單」，不承載處理狀態。其下方永遠保留單一固定高度資訊區，統一顯示初始提示、延遲處理、成功／部分失敗、取消、清空及移除復原，避免狀態切換造成版面位移。
- 新增開始後立即鎖定檔案選擇與清空操作，但前 300ms 保留原本外觀與既有資訊；若尚未完成才在固定資訊區顯示 spinner、進度文字與「取消本次新增」。快速批次直接切換到結果，不閃現處理文字或 disabled 外觀。
...
- 預覽僅呈現目前選取檔案及分頁資料。
- 解壓、解析、正規化、驗證、輸出檢查與序列化都在單一 dedicated worker 內完成；主執行緒只保存清單摘要及目前最多 100 列的預覽頁。清空主要工作區會清除 worker 內的主要檔案與預覽 cache，但保留 Section 3 參照 Excel；舊結果不得回寫。
...
- 格式摘要、固定資訊區、預覽、提示、下載狀態與進階輸出共用短暫淡入淡出；只在語意狀態改變時觸發，不因每次進度數字或一般 render 重播，也不位移內容。
