...
- 新增、取消、移除、復原與清空各自具有明確且互斥的操作狀態。移除與新增共用 300ms 延遲回饋：快速完成時直接顯示結果，超過 300ms 才顯示處理標題與 spinner。連續移除會立即把說明更新為新檔名並停用原本的「復原」，但不重播動畫；移除與復原仍須先在 worker 完成，再公開對應的清單與自動預覽。清空可中止任何主要工作區操作，舊操作結果不得回寫。
- 新增結果只摘要該次加入目前格式與「其他檔案」的數量，不重複工作區總數。未加入項目預設收合，只列友善原因與檔名；展開後以資訊區錨定的浮動面板覆蓋下方內容並在面板內捲動，不推動資訊文字或按鈕。點擊面板外或按 Escape 會收合；一般 UI 不顯示 parser、codec、bytes、代碼或 stack。
- 「全部標示為已查看」、處理取消與移除復原都是固定資訊區的情境操作。資訊區右側保留兩個固定操作欄位：全部已查看固定在左欄，只要仍有未查看檔案便持續顯示；右欄只顯示當下操作，包括未加入明細、取消或復原。桌面維持兩個固定欄位，中等寬度改為兩個等寬欄位，最窄時依 DOM 順序堆成單一全寬欄；單一動作在桌面使用右欄。檔名、數量、按鈕顯示與隱藏都不得改變欄位位置。移除中保留「復原」但停用，完成後才啟用；處理與取消不移動一般完成焦點，取消完成後焦點回到「新增檔案」。
...
- 下載卡保留既有狀態標題，並以一般說明文字顯示格式、輸出檔案、已勾選列與略過檔案摘要；不重複加入檔案、等待或按下載按鈕等已由流程、狀態與控制項表達的指示。問題或提醒預設收合，由與預覽相同的 `+／−` disclosure 展開完整清單；只有無法建立或檢查輸出的 runtime error 直接取代摘要顯示可採取行動的說明。
- 背景檢查或建立下載時，在固定下載狀態標題旁顯示 spinner 並停用下載；完成後沿用同一位置更新結果，不造成版面位移。
...
- 讀取參照 Excel、整理查詢結果或建立進階 XLSX 時，在對應的固定狀態標題旁顯示 spinner 並停用下載；動畫遵守 reduced-motion。
...
- 檔案選擇、檔案操作、標準下載與進階下載使用同一種操作卡片階層：標題、說明與右側兩個等寬操作欄位固定對齊；只有一個下載按鈕時放在最右欄。桌面 action rail 對齊標題列，不因下方摘要、說明或 disclosure 展開而重新置中；窄畫面依閱讀順序自然換行。主要操作保留一般控制高度，只有預覽分頁使用同色彩、邊框與焦點樣式的較小按鈕。載入 spinner 在標題旁保留固定 inline 空間，顯示或隱藏不得改變文字、卡片高度或按鈕位置。
- 動畫遵守 reduced-motion，狀態不只以顏色表達。
- 格式摘要、固定資訊區、預覽、提示、下載狀態與進階輸出共用同一個短暫 settle transition：舊內容保留至新狀態可一次提交，新內容只從接近完整的不透明度淡入，不先淡出、不動畫 `display`、visibility、尺寸或位置。只在語意狀態或目前處理檔名改變時觸發，不因進度數字或一般 render 重播；reduced-motion 時立即更新。
