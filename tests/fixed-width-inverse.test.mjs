...
test("parses left-aligned fixed-width Big5 records and skips physical blank lines", () => {
  ...
  const parsed = parseFixedWidthBig5(source, [4, 4]);
...
test("rejects wrong record widths, unsafe Big5 bytes, and unpaired CR bytes", () => {
  assert.match(
    parseFixedWidthBig5(new Uint8Array([0x41, 0x0a]), [2]).errors[0] ?? "",
    /第 1 筆共有 1 位元組，應為 2 位元組/u,
  );
  assert.match(
    parseFixedWidthBig5(new Uint8Array([0xff]), [1]).errors[0] ?? "",
    /無法安全解讀的 Big5 位元組/u,
  );
  assert.match(
    parseFixedWidthBig5(new Uint8Array([0x0d]), [1]).errors[0] ?? "",
    /未配對的 CR/u,
  );
});
