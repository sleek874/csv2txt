...
The generated synthetic CSV fixtures are documented in
[`tests/fixtures/README.md`](tests/fixtures/README.md). They contain no real
personal or production data.

Use `npm install` only when intentionally adding or updating dependencies so
that `package-lock.json` is updated with `package.json`.

The current application shell is a Traditional Chinese configuration prototype.
It intentionally contains no file conversion logic yet.
