...
檔案內容、路徑、可讀欄名、內部資料、驗證結果與輸出都不會上傳。除主題設定外，網站只在本機保存無法還原文字的加鹽欄名指紋，用來記住進階輸出的欄位選擇。
...
- Section 2 顯示第 0 區已選的格式、輸出／略過檔案與列摘要、下載及取消按鈕；問題明細預設收合。零勾選檔案不建立輸出；多檔 ZIP 最多 5,000 個 100 MiB entries，最終上限 500 MiB。BIG-5E TXT 以官方 BIG5-2003＋BIG-5E 對照逐字輸出；無 mapping 字元以全形 `？` 代替且不阻擋，替代後超過固定欄寬才停止下載。
- 本機處理、CSP、iframe 防護與離線快取。
- Excel 程式碼與預覽字型的延遲載入。
- Section 3 可另選一個有標題列的 Excel，以欄位11查詢使用者選定的參照欄、加入指定欄位，並下載單一整理後 XLSX；提醒明細預設收合，重複與未命中不阻止下載。查詢欄與附加欄只以 salted SHA-256 header fingerprints 保存在目前網站。
- 共用視覺、響應式與可及性基礎，以及對應的自動檢查。
