...
- `csv/`、`xls/`、`xlsx/`、`txt/`：四個目錄具有相同的 dataset basename、列數、順序與 15 欄值。CSV 為 UTF-8、TXT 為 Big5 固定 208 bytes／列；所有檔案都沒有標題列。
- `zip/`：四個情境 ZIP，各自混合 CSV、XLS、XLSX 與 TXT，並保留安全的相對路徑。
- `manifest.json`：機器可讀的欄位 mock 名稱、dataset 類型、列數、預期摘要與 ZIP entry 清單。

最大檔案為 `clean-large-6000`（6,000 列）。目前共 44 個資料檔，低於 300 個上限。

## 推定欄位名稱

這些名稱只協助閱讀 mock data，是依目前 regex、byte 寬度及 validation hook 推定的測試語意；正式契約仍以「欄位1」至「欄位15」的位置規則為準，UI 不採用業務欄位名稱。

| 欄位 | Mock 名稱 | Big5 bytes | 主要線索 |
|---:|---|---:|---|
| 1 | 資料類別 | 1 | `A`／`B` |
| 2 | 區域／機構代碼 | 2 | 2 位數字 |
| 3 | 資料子類型 | 1 | `1` 至 `6` |
...
| 7 | 姓名／名稱 | 12 | 必填 Big5 文字 |
| 8 | 性別代碼 | 1 | `1`／`2`，與有效欄位5連動 |
| 9 | 地址 | 120 | 必填 Big5 文字、最寬欄位 |
...
- `modified-phone-default`：欄位10來源為空，驗證流程應留下補值修改紀錄。
- `warning-optional-id`：欄位5符合 regex 但 checksum 無效，只有 warning。
- `error-validation`：涵蓋 regex、日期、checksum、性別及欄位14／15跨欄錯誤；所有值仍可安全放入 Big5 固定欄寬，使四種格式可保持同值。
